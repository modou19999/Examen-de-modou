...
import express from 'express'; // impporto express para crear el el servidor

import { mockProducts } from './mockproducto.ts'; // importo el mock de productos

import type { Request, Response } from 'express';

// aquí Creo la instancia de Express
const app = express();

app.use(express.json());

// almacenar los products en memoria
let products = [...mockProducts];

// endopoint GET product
...
app.delete('/products/:id', (req: Request, res: Response) => {
    ...
    if (productIndex !== -1) {
        ...
        res.status(204).send(); // Responder código 204 (No Content)
    } else {
        res.status(404).json({ message: 'Product not found' }); // Si no encuentra, responder con error 404
    }
});
